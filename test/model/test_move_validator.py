...
class TestMoveValidator(TestCase):
    ...
    def test_pawn_blocked_at_front(self):
        ...
        self.assertSetEqual({(1, 2)}, generate_move(board=board, coord=(1, 1)))

        board.clear()
        board[6][0].piece = Pawn("WHITE")
        board[5][0].piece = Pawn("BLACK")
        self.assertSetEqual(set(), generate_move(board=board, coord=(0, 6)))

        board.clear()
        board[6][1].piece = Pawn("WHITE")
        board[4][1].piece = Pawn("BLACK")
        self.assertSetEqual({(1, 5)}, generate_move(board=board, coord=(1, 4)))

        board.clear()
        board[6][0].piece = Pawn("BLACK")
        self.assertSetEqual({(0, 7)}, generate_move(board=board, coord=(0, 6)))
        board[1][0].piece = Pawn("WHITE")
        self.assertSetEqual({(0, 0)}, generate_move(board=board, coord=(0, 1)))
    ...
    def test_pawn_take_move(self):
        ...
        self.assertIn((2, 2), generate_move(board=board, coord=(1, 1)))
        ...
        # 2 PW |    | PW
        board[2][0].piece = Pawn("WHITE")
        self.assertIn((0, 2), generate_move(board=board, coord=(1, 1)))
    ...
    def test_bishop_basic_move(self):
        ...
        actual_moveset: MoveSet = generate_move(board=board, coord=(2, 2))
        self.assertSetEqual(expected_moveset, actual_moveset)
